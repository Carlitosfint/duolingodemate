...
const code = `import { ProblemData } from '../types';
...
export function generateLogicaInferencial(isGolden: boolean): ProblemData {
  const t = rnd(0, 1);
  let intro, expected, explanation;
  ...
  if (isGolden) {
    intro += \` Suma 4 a tu respuesta.\`;
    expected += 4;
    explanation += \` Más 4 da \${expected}.\`;
  }
...
fs.writeFileSync('src/utils/math_razonamiento_5to.ts', code);
