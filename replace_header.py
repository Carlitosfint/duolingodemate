...
start_marker = "      {/* Top Header Dashboard HUD - Single Row */}"
...
new_header = """      {/* Top Header Dashboard HUD - Single Row */}
      <header className={`max-w-5xl mx-auto w-full flex flex-row justify-between items-center gap-1.5 md:gap-2 mb-4 z-30 relative shrink-0 transition-all duration-300`}>
        ...
        <div className={`flex items-center gap-2 md:gap-3 rounded-full border-2 p-1.5 md:p-2 pr-3 md:pr-4 shadow-sm ${currentThemeStyle.headerBg}`}>
          ...
          <div className="flex flex-col min-w-0 pr-1">
            ...
            <p className="text-[9px] md:text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-0.5 md:mt-1">Nvl {progress.level}</p>
          </div>
          
          <div className="flex items-center ml-2 bg-amber-400 rounded-full px-2 md:px-3 py-1 gap-1 border-b-[3px] border-amber-600">
        ...
        <div className={`flex items-center gap-1.5 md:gap-2 rounded-full border-2 p-1.5 md:p-2 shadow-sm ${currentThemeStyle.headerBg}`}>
           ...
           <button 
             ...
             className="w-9 h-9 md:w-11 md:h-11 rounded-full bg-amber-50 text-amber-600 flex items-center justify-center text-lg md:text-xl border-2 border-amber-200 hover:scale-105 transition-all shadow-sm relative"
             title="Álbumes"
           >
             🖼️
             <span className="absolute -top-1 -right-1 bg-red-500 text-white text-[8px] font-black w-4 h-4 flex items-center justify-center rounded-full border border-white">1</span>
           </button>

           <button 
             onClick={() => setViewMode('map')}
             className="w-9 h-9 md:w-11 md:h-11 rounded-full bg-rose-50 text-rose-600 flex items-center justify-center border-2 border-rose-200 hover:scale-105 transition-all shadow-sm ml-1"
