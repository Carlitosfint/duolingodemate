...
async function seedSchool(name: string, slug: string) {
  ...
  console.log(`Colegio creado: "${created.name}" (id ${created.id}, slug "${slug}").`);
  return created;
}

seedSchool('Colegio Ángeles de Jesús', 'angeles-de-jesus')
  .then(() => process.exit(0))
  .catch((err) => {
    console.error(err);
    process.exit(1);
