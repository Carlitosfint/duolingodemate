...
new_reveal = """export const PageReveal: React.FC<{ children: React.ReactNode, className?: string, isFullScreen?: boolean, bgClass?: string }> = ({ children, className, isFullScreen = false, bgClass = 'bg-slate-50' }) => {
  ...
  React.useEffect(() => {
    if (!isPresent) {
       // Exiting component: just wait until the entering one finishes its animation, then remove
       // The total animation takes about 3.5s
    ...
    const t3 = setTimeout(() => {
      animate(holeRadius, 50, { duration: 0.4, ease: "easeInOut" });
      animate(pageRadius, 50, { duration: 0.4, ease: "easeInOut" });
    }, 2000); 
    ...
    const t4 = setTimeout(() => {
      ...
      animate(holeRadius, maxR, { duration: 0.8, ease: [0.64, 0, 0.78, 0] });
      animate(pageRadius, maxR, { duration: 0.8, ease: [0.64, 0, 0.78, 0] });
    }, 2800); 
