...
export const ProfileModal: React.FC<ProfileModalProps> = ({
  ...
  const content = (
    <div className={\`w-full \${isInline ? 'h-full bg-white border-0 rounded-none overflow-hidden flex flex-col' : 'max-w-4xl bg-white border-[6px] border-slate-200 rounded-[2.5rem] md:rounded-[3rem] p-6 md:p-10 my-8 shadow-2xl animate-pop'} text-slate-800 relative\`}>
      <div className={\`flex flex-col md:flex-row justify-between items-center gap-4 mb-8 border-b-2 border-slate-100 pb-6 relative z-10 \${isInline ? 'p-8 pt-10 pb-6 shrink-0 mb-0' : ''}\`}>
      ...
      <div className={\`grid grid-cols-1 md:grid-cols-3 gap-6 relative z-10 no-scrollbar \${isInline ? 'flex-1 overflow-y-auto px-8 pb-8' : 'max-h-[55vh] overflow-y-auto pr-2'}\`}>          
